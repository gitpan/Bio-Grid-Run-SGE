...
# combination bug
{
    my $idx0 = Bio::Grid::Run::SGE::Index::Dummy->new( idx => [ 1 .. 42 ], idx_file => '/tmp/dummy_idx.0' )->create;
    is( $idx0->num_elem, 42 );
    my $idx1 = Bio::Grid::Run::SGE::Index::Dummy->new( idx => [ 1 .. 55 ], idx_file => '/tmp/dummy_idx.1' )->create;
    is( $idx1->num_elem, 55 );
...

{
    ...
    my @files = ( 't/data/test.fa', 't/data/Bio-Grid-Run-SGE-Index-General.range-44-44-0.ref.fa',
        't/data/test.fa' );
    $idx1->create( \@files );

    is( $idx1->num_elem, 3 );

    my $idx0 = Bio::Grid::Run::SGE::Index::Dummy->new( idx => [ 1 .. 42 ], idx_file => '/tmp/dummy_idx.10' )->create;
    is( $idx0->num_elem, 42 );
